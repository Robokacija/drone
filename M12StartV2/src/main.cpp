...
//DEFINES:
//
#define MOTOR_PIN_1_A 18
#define MOTOR_PIN_1_B 19
#define MOTOR_PIN_2_A 32
#define MOTOR_PIN_2_B 33
#define MOTOR_PIN_3_A 25
#define MOTOR_PIN_3_B 26
#define MOTOR_PIN_4_A 27
#define MOTOR_PIN_4_B 13


const char* ssid = "IKdrone";
...
<body>
 ...
 <h3>Motor Control for Testing</h3>
  PWM1: <input type="range" min="0" max="255" step="1" id="pwm1"><span id="pwm1Val">0</span><br>
  PWM2: <input type="range" min="0" max="255" step="1" id="pwm2"><span id="pwm2Val">0</span><br>
  PWM3: <input type="range" min="0" max="255" step="1" id="pwm3"><span id="pwm3Val">0</span><br>
  PWM4: <input type="range" min="0" max="255" step="1" id="pwm4"><span id="pwm4Val">0</span><br>
...
void setup(void) {
  ...
  analogWrite(MOTOR_PIN_1_A,120);
  analogWrite(MOTOR_PIN_1_B,0);
   delay(100);
  analogWrite(MOTOR_PIN_2_A,120);
  analogWrite(MOTOR_PIN_2_B,0);
   delay(100);
  analogWrite(MOTOR_PIN_3_A,120);
  analogWrite(MOTOR_PIN_3_B,0);
   delay(100);
  analogWrite(MOTOR_PIN_4_A,120);
...
void loop() {
  ...
  InputX = (atan(a.acceleration.y / a.acceleration.z)) * 180 / PI ;
  InputY = (atan(a.acceleration.x / a.acceleration.z)) * 180 / PI ;
...
// Calculate motor outputs based on PID outputs
  ...
  int motorSpeed1 = constrain(pwm1 - OutputX + OutputY + OutputZ, 0, 255); // Front-left motor (M1)
  int motorSpeed2 = constrain(pwm2 + OutputX + OutputY - OutputZ, 0, 255); // Front-right motor (M2)
  int motorSpeed3 = constrain(pwm3 + OutputX - OutputY + OutputZ, 0, 255); // Back-right motor (M3)
  int motorSpeed4 = constrain(pwm4 - OutputX - OutputY - OutputZ, 0, 255); // Back-left motor (M4)
