 ...
 // PWM control routes
  server.on("/setPWM1", HTTP_GET, [](AsyncWebServerRequest *request){
    if (request->hasParam("value")) {
      ...
      analogWrite(MOTOR_PIN_1_A, pwm);
      request->send(200, "text/plain", "PWM1 set to: " + String(pwm));
    } else {
  ...
  server.on("/setPWM2", HTTP_GET, [](AsyncWebServerRequest *request){
    if (request->hasParam("value")) {
      int pwm = request->getParam("value")->value().toInt();
      analogWrite(MOTOR_PIN_2_A, pwm);
      request->send(200, "text/plain", "PWM2 set to: " + String(pwm));
    } else {
  ...
  server.on("/setPWM3", HTTP_GET, [](AsyncWebServerRequest *request){
    if (request->hasParam("value")) {
      int pwm = request->getParam("value")->value().toInt();
      analogWrite(MOTOR_PIN_3_A, pwm);
      request->send(200, "text/plain", "PWM3 set to: " + String(pwm));
    } else {
  ...
  server.on("/setPWM4", HTTP_GET, [](AsyncWebServerRequest *request){
    if (request->hasParam("value")) {
      int pwm = request->getParam("value")->value().toInt();
      analogWrite(MOTOR_PIN_4_A, pwm);
      request->send(200, "text/plain", "PWM4 set to: " + String(pwm));
    } else {
...
void loop() {
  ...
  InputY = (atan(a.acceleration.z / a.acceleration.z)) * 180 / PI ;
